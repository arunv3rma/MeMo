"""
NarrativeQA Evaluation Script — No Context
===========================================
Variant of main_for_narrativeqa.py that sends ONLY the raw question to the
model, with no story or summary. Useful as a closed-book / parametric-knowledge
baseline to compare against the full-story version.

Answers are batch-judged using run_evaluation_multi_answer from
...
async def collect_answers(args, questions: list[dict]) -> list[dict]:
    ...
    tasks = [
        process_entry(client, args.model, entry, idx, args.max_tokens, answer_semaphore)
    ...
    for coro in async_tqdm(
        ...
        total=len(tasks),
        desc=f"Collecting answers from {args.model} on NarrativeQA (no context)",
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate an OpenRouter model on NarrativeQA (no context, closed-book)"
    )
    ...
    parser.add_argument(
        ...
        help="OpenAI-compatible API base URL (e.g. http://localhost:4323/v1 for vLLM).",
    )
    parser.add_argument(
    ...
    for split in splits_to_run:
        print(f"\n{'=' * 60}")
        print(f"NarrativeQA split: {split}  (no context)")
        print(f"{'=' * 60}")

        output_path = os.path.join(model_dir, f"narrativeqa_{split}_results_no_context.json")
        metrics_path = os.path.join(model_dir, f"narrativeqa_{split}_metrics_no_context.json")
        ...
        print(f"Context         : NONE (closed-book baseline)\n")
        ...
        print(f"\nRESULTS — NarrativeQA-{split}  (no context)")
    ...
    if len(splits_to_run) > 1:
        print(f"\n{'=' * 60}")
        print(f"OVERALL SUMMARY  (no context)")
        print(f"{'=' * 60}")
        ...
        combined_path = os.path.join(model_dir, "narrativeqa_all_metrics_no_context.json")
