...
Outputs land in `baselines/output_seeded/` and logs in `baselines/output_seeded/logs/` — both auto-created.
...
bash baselines/scripts/run_bm25_bcp_qwen_seeds.sh
```
...
**Per-method, per-benchmark seed sweeps (Qwen2.5-32B-Instruct serving):**
- BM25: `run_bm25_{bcp,musique,nqa}_qwen_seeds.sh`
- HippoRAG2: `run_hipporag2_{bcp,musique,nqa}_qwen_seeds.sh`
- NV-Embed: `run_nv_embed_{bcp,musique,nqa}_qwen_seeds.sh`
- Cartridges: `run_cartridges_{bcp,musique,nqa}_qwen_seeds.sh`
...
**Helpers:**
- `aggregate_seeds.py` — combines per-seed `*_seed{1,2,3,...}_*.json` outputs into a single `combined_<method>_<bench>_<model>_<timestamp>.json`. Called automatically at the end of each seed-sweep script.
