"""
Naive multi-turn extension of eval_msq_trng.py (single-turn).
...
async def main():
    ...
    question_to_no = {item['question']: item.get('question_no', '') for item in output}
    ...
    eval_results = run_evaluation_multi_answer(
        ...
        run_id="eval-msq-naive-multi-turn"
    )
    ...
    hop_accuracy = {hop: hop_correct[hop] / hop_total[hop] for hop in hop_total}
    ...
    print(f"Overall accuracy: {summary_output['summary']['overall_accuracy']:.3f}")
    for hop, acc in sorted(hop_accuracy.items()):
        print(f"  {hop}: {acc:.3f} ({hop_correct[hop]}/{hop_total[hop]})")
    print("=" * 80)
