...
from bcp_query_subset_utils import SUBSET_300_QUERY_IDS
from bcp_query_negatives_utils import NEGATIVE_DOC_IDS_PER_QUERY, NEGATIVE_DOCS_MAP
SUBSET_MAP = {
    300 : SUBSET_300_QUERY_IDS,
}
...
def load_questions_with_evidence_docs(file_path, max_valid_questions=None):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line_num, line in enumerate(f):
            ...
            if max_valid_questions is not None and data['query_id'] not in SUBSET_MAP[max_valid_questions]:
                invalid_question_count += 1
                continue
...
def load_only_query_related_docs(corpus_path, qns_path, max_valid_questions=None):
    def get_only_query_related_docs(qns_path, max_valid_questions=None):
        """
        Loads questions with their evidence documents directly from JSONL file.
        No retrieval step - just uses all evidence_docs for each question.
        ...
        Returns:
            questions (list): List of question entries with evidence docs already attached
            all_docs (list): Deduplicated list of doc objects {doc_id, text, url} 
                            across all evidence_docs, gold_docs, and negative_docs
        """
        valid_question_count = 0
        invalid_question_count = 0
        query_doc_ids = set()
        ...
        with open(qns_path, 'r', encoding='utf-8') as f:
            for line_num, line in enumerate(f):
                ...
                # handling the case where max_valid questions is specified
                if max_valid_questions is not None and data['query_id'] not in SUBSET_QUERY_IDS:
                    invalid_question_count += 1
                    continue
                
                # handling the case where max_valid_questions is not specified
                if max_valid_questions is None and data['query_id'] in BROWSECOMP_QUERY_IDS_TO_SKIP:
                    invalid_question_count += 1
                    continue
                ...
                # Collect unique docs across all three sources
                # for doc in evidence_docs + gold_docs + negative_docs:
                for doc in evidence_docs + gold_docs:
                    doc_id = doc.get('docid')
                    if doc_id and doc_id not in query_doc_ids:
    ...
    query_doc_ids = get_only_query_related_docs(qns_path, max_valid_questions)
    ...
    del corpus_dict  # Free the dict now that we've extracted what we need
    gc.collect()
    # Print summary
    print(f"\n=== Corpus Loading Summary ===")
    print(f"Total lines processed: {line_num}")
...
def load_only_query_related_docs_with_negatives(corpus_path, qns_path, max_valid_questions=None, neg_n=1):
    def get_only_query_related_docs_with_negatives(qns_path, max_valid_questions=None):
        """
        Loads questions with their evidence documents directly from JSONL file.
        No retrieval step - just uses all evidence_docs for each question.
        ...
        with open(qns_path, 'r', encoding='utf-8') as f:
            for line_num, line in enumerate(f):
                ...
                pre_selected_neg_ids = NEGATIVE_DOCS_MAP[neg_n].get(data['query_id'], [])
    ...
    query_doc_ids = get_only_query_related_docs_with_negatives(qns_path, max_valid_questions)
...
def load_questions_with_evidence_and_negative_docs(file_path, max_valid_questions=None):
    """
    Loads questions with their evidence documents directly from JSONL file.
    No retrieval step - just uses all evidence_docs for each question.
