"""
generate_surface_entity_cache.py

Takes the output of check_self_containment_post_combination.py and, for each
doc_id, surfaces entity QA pairs by passing ALL self-contained QA pairs for
that doc together — enabling multi-fact entity profiling across the full
document rather than one QA pair at a time.

- Groups all self-contained QA pairs per doc_id from the input
- Distributes docs across N clients by round-robin (doc index % total_clients)
- Each doc produces one entity-surfacing call
- Resume via a checkpoint file

Output format is the same qa_pairs_cache structure consumed by
generate_paraphrase_cache.py: each entry has doc_id and a qa_pairs list
of {question, answer} dicts.

Usage:
    python generate_surface_entity_cache.py \\
        --ports 8000 8001 8002 \\
        --input_file_path /path/to/self_containment_output.json \\
        --output_file_path /path/to/surface_entity_cache.json \\
...
from general_prompt_utils import extract_doc_metadata, prepare_prompt_for_entity_surfacing
...
class RunState:
    ...
    def completed_keys(self) -> set[str]:
        """Set of doc_ids already processed (including permanent errors)."""
...
def load_resume_checkpoint(path: str) -> tuple[list[dict], dict]:
    ...
    try:
        with open(path, "r", encoding="utf-8") as f:
            data = json.load(f)
        results  = data.get("qa_pairs_cache", [])
...
# ---------------------------------------------------------------------------
# Core: surface entities for a full doc's QA pairs
# ---------------------------------------------------------------------------

async def generate_surfacing_for_doc(
    doc_id: str,
    qa_pairs_str: str,
    doc_date: str,
    source_qa_count: int,
    ...
    async with semaphore:
        print(f"[Client {client_id}][Doc {doc_id}] Starting entity surfacing ({source_qa_count} source QA pairs)...")
        gen_start = time.time()

        prompt = prepare_prompt_for_entity_surfacing(
        ...
        result = await query_with_hedging(
            ...
            request_id=f"Client{client_id}_Doc_{doc_id}_SurfaceEntity",
            ...
            prompt_setting=PromptSetting.SurfaceEntity,
            stream=args.stream,
        ...
        if isinstance(result, tuple) and len(result) >= 2:
            ...
            entry = {
                "doc_id":   doc_id,
                "qa_pairs": original_qa_pairs if args.include_source_qa_pairs else [],
        ...
        if not result:
            ...
            entry = {
                "doc_id":   doc_id,
                "qa_pairs": original_qa_pairs if args.include_source_qa_pairs else [],
                "error":    {"type": "EMPTY_RESPONSE", "message": "No entity surfacing QA pairs generated"},
                "timing":   {"generation": gen_elapsed, "total": time.time() - start},
            }
            await state.add_result(entry)
            return entry

        # Extract the list of surfaced QA pairs — strip metadata fields,
        # keep only question + answer for downstream compatibility
        raw_pairs = result.get("entity_surfacing_qa_pairs", []) if isinstance(result, dict) else []
        surfaced_qa_pairs = [
        ...
        print(
            ...
            f"Done — {len(surfaced_qa_pairs)} surfaced pairs in {total_elapsed:.2f}s"
        )

        # Merge with source QA pairs if requested, otherwise only surfaced pairs
        output_qa_pairs = (
            original_qa_pairs + surfaced_qa_pairs
            if args.include_source_qa_pairs
            else surfaced_qa_pairs
        )
...
async def run_client(
    client_id: int,
    port: int,
    work_items: list[tuple[str, str, str, int, list[dict]]],
    args: argparse.Namespace,
    state: RunState,
    checkpoint_path: str,
) -> None:
    ...
    async def _safe(doc_id: str, qa_pairs_str: str, doc_date: str, source_qa_count: int, original_qa_pairs: list[dict]) -> None:
        try:
            await generate_surfacing_for_doc(
                doc_id=doc_id,
                qa_pairs_str=qa_pairs_str,
                doc_date=doc_date,
...
def load_and_group_qa_pairs(input_path: str) -> dict[str, list[dict]]:
    """
    Load the self-containment cache and group all self-contained QA pairs
    by doc_id. Only entries with is_self_contained=True are included.
    """
    ...
    for entry in entries:
        ...
        if not doc_id:
            continue
        if not entry.get("is_self_contained", False):
            continue

        qa_pair = {
            "question": entry.get("final_question", ""),
        ...
        if doc_id not in grouped:
            grouped[doc_id] = []
        grouped[doc_id].append(qa_pair)

    return grouped


def format_qa_pairs_for_prompt(qa_pairs: list[dict]) -> str:
    """Convert a list of QA pair dicts into a numbered string for the prompt."""
    lines = []
...
async def main(args: argparse.Namespace) -> None:
    # ---- Passthrough mode --------------------------------------------------
    if args.passthrough:
        print("[PASSTHROUGH] Skipping entity surfacing — passing self-contained QA pairs through unchanged...")
        grouped_qa = load_and_group_qa_pairs(args.input_file_path)
        results = [{"doc_id": doc_id, "qa_pairs": qa_pairs} for doc_id, qa_pairs in grouped_qa.items()]
    ...
    print("Loading and grouping self-contained QA pairs from input file...")
    grouped_qa = load_and_group_qa_pairs(args.input_file_path)
    print(f"  Found {len(grouped_qa)} docs with self-contained QA pairs\n")
    ...
    if args.corpus_path:
        ...
        print("  Warning: no --corpus_path provided; doc_extracted_date will be 'No date provided in document'\n")
    ...
    total_ok     = sum(1 for r in state.results if "error" not in r)
    ...
    print(f"  Total qa_pairs  : {total_pairs}")
    ...
    output = {"qa_pairs_cache": state.results}
    with open(args.output_file_path, "w", encoding="utf-8") as f:
        json.dump(output, f, indent=2)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Entity surfacing QA generation from self-containment cache")
    ...
    p.add_argument("--input_file_path",   type=str, required=True,
                   help="Path to output JSON from check_self_containment_post_combination.py")
    ...
    # Filtering
    p.add_argument("--min_qa_pairs", type=int, default=1,
                   help="Minimum self-contained QA pairs a doc must have to attempt surfacing")

    # Output
    p.add_argument("--include_source_qa_pairs", action="store_true",
                   help="If set, merges the original input QA pairs with the newly generated "
                        "surfaced entity QA pairs in the output. If not set, only the surfaced "
                        "entity QA pairs are written out.")
    ...
    p.add_argument("--passthrough", action="store_true",
                   help="Skip entity surfacing and pass self-contained QA pairs through unchanged (for ablation studies)")
