"""
generate_crossdoc_entity_combination_cache.py
...
Usage:
    python generate_crossdoc_entity_combination_cache.py \\
        --ports 4325 4327 \\
...
async def main(args: argparse.Namespace) -> None:
    # ---- Passthrough mode --------------------------------------------------
    if args.passthrough:
        print("[PASSTHROUGH] Skipping cross-doc combination — passing source entries through unchanged...")
        original_entries, _ = load_surface_entity_cache(args.input_file_path)
        output_entries = original_entries if args.include_source_qa_pairs else []
        ...
        print(f"[PASSTHROUGH] Wrote {len(output_entries)} entries → {args.output_file_path}")
        return
    ...
    if args.dataset == "nqa":
        questions = load_questions_with_evidence_docs_nqa(args.qns_path, args.max_num_questions)
    elif args.dataset == "musique":
        questions = load_questions_with_evidence_docs_musique(args.qns_path, args.max_num_questions)
    elif args.dataset == "lveval":
        questions = load_questions_with_evidence_docs_lveval(args.qns_path, args.max_num_questions)
    else:
        questions = load_questions_with_evidence_docs(args.qns_path, args.max_num_questions)
    print(f"  Questions loaded: {len(questions)}\n")
    valid_query_ids = {str(q["question_no"]) for q in questions}
    ...
    subset_results = [r for r in state.results if r.get("query_id") in valid_query_ids]
    aggregated = aggregate_results_by_query(subset_results)
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--dataset", type=str, default="bcp", choices=["bcp", "nqa", "musique", "lveval"],
                   help="Dataset type: 'bcp' (BrowseComp+), 'nqa' (NarrativeQA), 'musique' (MuSiQue), or 'lveval' (LV-Eval)")
    ...
    p.add_argument("--passthrough", action="store_true",
                   help="Skip cross-doc combination and pass source entries through unchanged (for ablation studies)")
