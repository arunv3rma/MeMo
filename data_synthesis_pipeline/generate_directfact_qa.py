...
Usage:
    python generate_directfact_qa.py \
...
from general_prompt_utils import prepare_prompt_for_direct_fact_extraction_v3, extract_doc_metadata
from bcp_data_utils import load_corpus_from_jsonl, load_only_query_related_docs
from nqa_data_utils import load_corpus_from_jsonl_nqa, load_only_query_related_docs_nqa
from musique_data_utils import load_corpus_from_jsonl_musique, load_only_query_related_docs_musique
...
async def generate_qa_for_chunk(
    ...
    async with semaphore:
        ...
        prompt_content = prepare_prompt_for_direct_fact_extraction_v3(
...
async def main(args: argparse.Namespace) -> None:
    # ---- Passthrough mode --------------------------------------------------
    if args.passthrough:
        print("[PASSTHROUGH] Skipping LLM generation — writing empty direct-fact QA entries...")
        if args.dataset == "nqa":
            corpus_texts, corpus_docids = load_only_query_related_docs_nqa(
            ...
            corpus_texts, corpus_docids = load_only_query_related_docs_lveval(
                args.corpus_path, args.qns_path, args.max_num_questions
            )
        else:
