"""
BrowseComp+ Evaluation Script — With Evidence Documents
========================================================
Evaluates any OpenAI-compatible model against the BrowseComp+ benchmark.

For each question all associated evidence documents are injected into the
prompt as context and the model is asked to answer in free text. Answers are
batch-judged using run_evaluation from evaluation_pipeline/deepeval_utils.py.
...
Instructions:
- Use the documents to answer the question directly.
- Think step by step before answering.
- Be concise and direct in your final answer.
"""


def format_docs(evidence_docs: list[dict], max_chars: int) -> str:
    parts = []
...
def build_answer_prompt(question: str, evidence_docs: list[dict], max_chars: int) -> str:
    docs_text = format_docs(evidence_docs, max_chars)
    return ANSWER_PROMPT.format(docs_text=docs_text, question=question)
...
async def process_entry(
    ...
    prompt = build_answer_prompt(entry["question"], entry["evidence_docs"], doc_max_chars)
...
async def collect_answers(args, questions: list[dict]) -> list[dict]:
    ...
    tasks = [
        process_entry(
            ...
            args.max_tokens, args.doc_max_chars, answer_semaphore,
        )
    ...
    for coro in async_tqdm(
        ...
        total=len(tasks),
        desc=f"Collecting answers from {args.model} on BrowseComp+",
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate a model on BrowseComp+ (with evidence documents, LLM-judged)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--doc_max_chars", type=int, default=4000,
        help="Truncate each evidence document to this many characters.",
    )
    args = parser.parse_args()
    args.model = resolve_model(args.model, args.base_url)

    print(f"\n{'=' * 60}")
    print("BrowseComp+ Evaluation  (with evidence documents)")
    print(f"{'=' * 60}")
    ...
    output_path = args.output or os.path.join(model_dir, "bcp_results.json")
    metrics_path = args.metrics_output or os.path.join(model_dir, "bcp_metrics.json")
    ...
    print(f"Doc max chars   : {args.doc_max_chars}\n")
    ...
    print(f"\n{'=' * 60}")
    print("RESULTS — BrowseComp+  (with evidence documents)")
    print(f"{'=' * 60}")
