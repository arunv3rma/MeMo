"""
MuSiQue Evaluation Script — With Supporting Paragraphs, OpenRouter
===================================================================
Evaluates any model available on OpenRouter against the MuSiQue benchmark
(dgslibisey/MuSiQue) as a multi-hop QA task.

For each question the model is given the gold supporting paragraphs as context
and asked to answer in free text. Answers are then batch-judged using
run_evaluation from evaluation_pipeline/deepeval_utils.py.

Uses the validation split, sampling 100 questions per hop type (300 total):
  2-hop : 100  (from 1252)
  3-hop : 100  (from  760)
  4-hop : 100  (from  405)
...
DEFAULT_QUESTIONS_FILE = os.path.join(_THIS_DIR, "musique_questions_chunks_1000.jsonl")
DEFAULT_CORPUS_FILE = os.path.join(
    _THIS_DIR, "../narrativeqa/hipporag2_dataset/musique.json"
)
...
ANSWER_PROMPT = """\
Use the following passages to help answer the question, along with your own knowledge.
...
Instructions:
- Use the passages as a guide, but also draw on your own knowledge where helpful.
...
def format_passages(paragraphs: list[dict], max_chars: int) -> str:
    parts = []
...
def build_answer_prompt(question: str, supporting_paras: list[dict], para_max_chars: int) -> str:
    passages_text = format_passages(supporting_paras, para_max_chars)
    return ANSWER_PROMPT.format(passages_text=passages_text, question=question)
...
async def process_entry(
    ...
    paragraphs = corpus.get(qid, {}).get("paragraphs") or []
    supporting_paras = [p for p in paragraphs if p.get("is_supporting")]
    ...
    prompt = build_answer_prompt(question, supporting_paras, para_max_chars)
...
async def collect_answers(args, examples: list[dict], corpus: dict) -> list[dict]:
    ...
    tasks = [
        process_entry(
            ...
            args.max_tokens, args.para_max_chars,
            corpus, answer_semaphore,
        )
    ...
    for coro in async_tqdm(
        ...
        total=len(tasks),
        desc=f"Collecting answers from {args.model} on MuSiQue",
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate an OpenRouter model on MuSiQue (with supporting paragraphs, LLM-judged)"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--corpus_file", type=str, default=DEFAULT_CORPUS_FILE,
        help="Path to musique.json containing paragraphs keyed by id",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--para_max_chars", type=int, default=3000,
        help="Truncate each supporting paragraph to this many characters",
    )
    args = parser.parse_args()
    args.model = resolve_model(args.model, args.base_url)

    print(f"\n{'=' * 60}")
    print("MuSiQue Evaluation  (with supporting paragraphs)")
    print(f"{'=' * 60}")

    model_dir = args.model.replace("/", "_")
    os.makedirs(model_dir, exist_ok=True)
    output_path = args.output or os.path.join(model_dir, "musique_results.json")
    metrics_path = args.metrics_output or os.path.join(model_dir, "musique_metrics.json")
    ...
    print(f"Para max chars  : {args.para_max_chars}\n")

    # Step 1: collect model answers
    results = asyncio.run(collect_answers(args, examples, corpus))
    ...
    print(f"\n{'=' * 60}")
    print("RESULTS — MuSiQue  (with supporting paragraphs)")
    print(f"{'=' * 60}")
