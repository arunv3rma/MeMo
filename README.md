...
<p>
  ...
  <a href="https://arunv3rma.github.io/blogs/memo/"><img alt="Project" src="https://img.shields.io/badge/Project-Page-1f6feb?style=for-the-badge&logo=githubpages&logoColor=white"></a>
...
<p align="center">
  ...
  <br/><br/>
  <em>
    <b>Training (left):</b> a frozen <b>G<small>ENERATOR</small></b> model transforms the corpus into a reflection QA dataset used to SFT the <b>M<small>EMORY</small></b> model.
    <b>Inference (right):</b> the <b>E<small>XECUTIVE</small></b> model decomposes the user query into sub-queries and reasons over M<small>EMORY</small> responses through a three-stage protocol.
  </em>
</p>
...
MeMo satisfies all the desirable properties that no single prior method achieves simultaneously:
...
MeMo separates **knowledge** from **reasoning** across two cooperating models:
...
- **Memory Model (MEM)** — a compact 1.5B–14B model whose *parameters* encode the target corpus. Answers EM's sub-queries from internalised *reflections*, never seeing source documents at inference time.
...
```
      ...
      ║       │                              ║    ║       │                        ║
      ║       ▼                              ║    ║       ▼                        ║
      ║  1.  Data synthesis  (5-step)        ║    ║  Stage 1 · Grounding           ║
      ║       │                              ║    ║       │                        ║
      ║       ▼                              ║    ║       ▼                        ║
...
Accuracy (%) on three knowledge-intensive benchmarks.  
Executive models: **Qwen2.5-32B-Instruct** / **Gemini-3-Flash** · Memory model: **Qwen2.5-14B-Instruct**
...
- 🎯  **Up to +27 pts on NarrativeQA** when paired with Gemini-3-Flash, beating every retrieval baseline.
...
```
MeMo/
├── baselines/                  # All retrieval and ICL baselines
│   ├── bm25/                   # BM25 sparse retrieval
│   ├── cartridges/             # Cartridges baseline
│   ├── hipporag2/              # HippoRAG2 baseline
│   ├── icl/                    # Oracle ICL and closed-book (no MEM)
│   ├── nv_embed/               # NV-Embed-V2 dense retrieval
│   ├── scripts/                # Shared runner scripts
...
│   ├── data_subsets/           # Pre-generated subset & hard-negative doc IDs
│   ├── datasynth_pipeline/     # End-to-end pipeline shell scripts per dataset
│   ├── langdetect_processing/  # Language detection utilities
...
# LFM memory models only (separate environment)
...
The full pipeline runs in five stages. Each links to a detailed sub-README.

```
  Stage 1      Stage 2      Stage 3      Stage 4      Stage 5
    Data    ──▶   Data    ──▶   SFT     ──▶ Merging  ──▶ Evaluation
Preparation    Synthesis    Training    (optional)
```
...
Generate the reflection-QA dataset with the 5-step pipeline:
**fact extraction → consolidation → verification → entity surfacing → cross-document synthesis**

Each dataset has a self-contained shell script under [`data_synthesis_pipeline/datasynth_pipeline/`](data_synthesis_pipeline/datasynth_pipeline/). The pipeline can be parallelised across multiple vLLM servers — a sample launcher is provided at `vllm_serve_qwen2_5_32b_instruct.sh`.

> [!TIP]
> Pre-generated subset IDs and hard-negative document IDs used in the paper are in [`data_synthesis_pipeline/data_subsets/`](data_synthesis_pipeline/data_subsets/) and can be fed directly into the synthesis or evaluation scripts. Larger pre-generated artifacts are mirrored on HuggingFace.
...
| :--- | :--- | :--- |
| Single-turn baseline | [`evaluation_pipeline/single_turn_baseline/`](evaluation_pipeline/single_turn_baseline/) | EM only, no MEM |
| Unstructured multi-turn | [`evaluation_pipeline/unstructured_multi_turn_baseline/`](evaluation_pipeline/unstructured_multi_turn_baseline/) | Naive multi-turn loop |
| **Structured multi-turn** | [`evaluation_pipeline/structured_multi_turn/`](evaluation_pipeline/structured_multi_turn/) | **Full MEMO protocol** |
| ICL baselines | [`baselines/icl/`](baselines/icl/) | Oracle retrieval and closed-book (no MEM) |

→ See [`evaluation_pipeline/README.md`](evaluation_pipeline/README.md) and each `baselines/<dataset>/README.md` for script-level details.
