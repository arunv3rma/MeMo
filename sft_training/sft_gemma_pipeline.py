#!/usr/bin/env python3
"""
Supervised Fine-Tuning of Gemma3 models on QA pairs.

Reads the output from generate_crossdoc_entity_combination_cache.py and trains
on question/answer pairs using Gemma3's chat template.

Gemma3 requires eager attention (not flash_attention_2). This script hardcodes
attn_implementation='eager' to avoid the following warning/error:
  "It is strongly recommended to train Gemma3 models with the eager attention
   implementation instead of flash_attention_2."
...
Usage:
    ...
    accelerate launch --config_file accelerate_config.yaml mvp_sft_online_gemma_pipeline.py \\
        --data_path /path/to/crossdoc_cache.json \\
        --output_dir /path/to/output

    # Single GPU / debug:
    python mvp_sft_online_gemma_pipeline.py --data_path /path/to/crossdoc_cache.json --output_dir ./sft-out

Requirements:
    pip install torch transformers accelerate trl datasets deepspeed
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="SFT of Gemma3 models on crossdoc QA pairs")
    ...
    parser.add_argument("--output_dir", type=str, default="./gemma3-sft")
    ...
    # NOTE: flash_attn is intentionally NOT supported for Gemma3. The model requires
    # attn_implementation='eager'. See:
    ...
    parser.add_argument("--wandb_project", type=str, default="gemma3-sft",
...
def main():
    ...
    # 4. Load model
    # -------------------------------------------------------------------------
    # Gemma3 requires eager attention — flash_attention_2 is not supported and
    # will produce incorrect results or errors. Always use attn_implementation='eager'.
    print("\nNote: Using attn_implementation='eager' (required for Gemma3 models)")
    ...
    #
    # Gemma3 chat template uses <start_of_turn>model\n to delimit the assistant turn,
    # unlike Qwen which uses <|im_start|>assistant\n.
    ...
    print(f"  Attention impl:      eager (Gemma3 requirement)")
